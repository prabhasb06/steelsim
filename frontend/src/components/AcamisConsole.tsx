  ...
  return <main className="h-full overflow-y-auto p-5 lg:p-7" aria-label="ACAMIS operational intelligence">
    <div className="mx-auto max-w-7xl">
      ...
      <section className="mt-5 rounded-lg border border-industrial-700 bg-industrial-800/70"><div className="flex flex-col gap-2 border-b border-industrial-700 px-4 py-3 lg:flex-row lg:items-center lg:justify-between"><div><h2 className="text-sm font-bold text-white">Advisory Model Gateway</h2><p className="mt-1 text-xs text-gray-500">Optional BYOK reasoning layer. Credentials remain in backend memory only; ACAMIS policy gates retain control.</p></div><span className={`rounded border px-2 py-1 font-mono text-[9px] font-bold ${data?.model_gateway.connected ? severityClass('INFO') : 'border-industrial-600 text-gray-500'}`}>{data?.model_gateway.connected ? `VERIFIED · ${data.model_gateway.model}` : 'DETERMINISTIC CORE ACTIVE'}</span></div>
        <div className="grid gap-4 p-4 xl:grid-cols-2"><form onSubmit={connectModel} className="grid gap-3 sm:grid-cols-2"><label className="text-[10px] font-bold uppercase tracking-wider text-gray-500">Provider<select value={provider} onChange={event => { setProvider(event.target.value); if (event.target.value === 'GEMINI') { setModelName('gemini-3.6-flash'); setBaseUrl(''); } }} className="mt-1 w-full rounded border border-industrial-600 bg-industrial-900 p-2 text-xs normal-case text-gray-200"><option value="GEMINI">Google Gemini</option><option value="OPENAI_COMPATIBLE">OpenAI-compatible</option></select></label><label className="text-[10px] font-bold uppercase tracking-wider text-gray-500">Model<input list="acamis-model-catalog" value={modelName} onChange={event => setModelName(event.target.value)} className="mt-1 w-full rounded border border-industrial-600 bg-industrial-900 p-2 text-xs normal-case text-gray-200" /><datalist id="acamis-model-catalog">{(data?.model_gateway.available_models ?? []).map(model => <option key={model} value={model} />)}</datalist></label>{provider === 'OPENAI_COMPATIBLE' && <label className="text-[10px] font-bold uppercase tracking-wider text-gray-500 sm:col-span-2">Base URL<input value={baseUrl} onChange={event => setBaseUrl(event.target.value)} placeholder="https://provider.example/v1" className="mt-1 w-full rounded border border-industrial-600 bg-industrial-900 p-2 text-xs normal-case text-gray-200" /></label>}<label className="text-[10px] font-bold uppercase tracking-wider text-gray-500 sm:col-span-2">Transient API key<input type="password" autoComplete="off" value={providerKey} onChange={event => setProviderKey(event.target.value)} placeholder="Not saved to files or returned by the API" className="mt-1 w-full rounded border border-industrial-600 bg-industrial-900 p-2 text-xs normal-case text-gray-200" /></label><div className="flex gap-2 sm:col-span-2"><button disabled={busy || !providerKey || !modelName} className="rounded border border-cyan-700 bg-cyan-950/40 px-3 py-2 text-[10px] font-bold text-cyan-200 disabled:opacity-30">Test & connect</button>{data?.model_gateway.connected && <button type="button" disabled={busy} onClick={() => void invoke('model/disconnect')} className="rounded border border-industrial-600 px-3 py-2 text-[10px] font-bold text-gray-300">Disconnect</button>}</div>{data?.model_gateway.connected && <p className="text-[10px] leading-4 text-emerald-300 sm:col-span-2">{data.model_gateway.message}</p>}</form>
          <div className="rounded border border-industrial-700 bg-industrial-900/60 p-3"><div className="font-mono text-[10px] font-bold text-cyan-300">OPERATOR CHANNEL · ADVISORY ONLY</div>{data?.model_advisory && <div className="mt-2 rounded border border-cyan-900/70 bg-cyan-950/20 p-2 text-[10px] leading-4 text-cyan-100"><b>AUTONOMOUS MODEL REVIEW</b><div className="mt-1 whitespace-pre-wrap text-gray-300">{data.model_advisory.reply}</div></div>}<textarea value={chatMessage} onChange={event => setChatMessage(event.target.value)} rows={3} className="mt-2 w-full resize-none rounded border border-industrial-600 bg-industrial-950 p-2 text-xs text-gray-200" /><button disabled={busy || !data?.model_gateway.connected || !chatMessage.trim()} onClick={() => void askModel()} className="mt-2 rounded border border-blue-700 bg-blue-950/40 px-3 py-2 text-[10px] font-bold text-blue-200 disabled:opacity-30">Request model review</button>{chatReply && <div className="mt-3 max-h-36 overflow-y-auto whitespace-pre-wrap border-l-2 border-cyan-700 pl-3 text-xs leading-5 text-gray-300">{chatReply}</div>}</div></div>
      </section>
